...
import json
with open('buffer (2).json') as file:
    text = json.load(file)
...
singelObj = list(filter(lambda d: d['name']['text'] == 'Cell', text['cells']))
groupedObj = list(filter(lambda d: '.uml-class-box-rect' in d['attrs'], text['cells']))
pointer = list(filter(lambda d: 'connector' in d, text['cells']))

idDict = {}
def IDmatchOBJ(objName):
    idDict[list(filter(lambda d: d['attrs']['text']['text'] == objName, singelObj))[0]['id']] = dataDict[objName]
